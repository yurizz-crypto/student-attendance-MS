...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        {{-- Override SweetAlert2 defaults to match app design system --}}
        <style>
            /* Remove default focus ring on all swal buttons */
            .swal2-confirm:focus,
            .swal2-cancel:focus,
            .swal2-deny:focus { box-shadow: none !important; outline: none !important; }

            /* Suppress Swal's default styled-components-like button outline */
            button.swal2-confirm { box-shadow: none !important; }

            /* Toast: keep background transparent so our didOpen inline style wins */
    ...
    <body class="font-sans antialiased text-navy bg-base">
        ...
        {{-- Global toast handler for all Livewire CRUD operations --}}
        <script>
            // Shared base for all Swal popups — matches the app design system
            const swalBase = {
            ...
            const swalToast = Swal.mixin({
                ...
                didOpen: (toast) => {
                    ...
                    // Tint progress bar to match icon
            ...
            document.addEventListener('livewire:init', () => {
                Livewire.on('swal:success', ({ title, message }) => {
                    const toast = swalToast.fire({ icon: 'success', title, text: message });
                ...
                Livewire.on('swal:error', ({ title, message }) => {
                    swalDialog.fire({
                        icon: 'error',
                        title,
                        text: message,
                        ...
                        didOpen: (popup) => {
                            ...
                            const title = popup.querySelector('.swal2-title');
                            if (title) { Object.assign(title.style, { fontWeight: '700', fontSize: '1.15rem', color: '#0F172A' }); }
                            const text = popup.querySelector('.swal2-html-container');
                            if (text) { Object.assign(text.style, { fontSize: '0.9rem', color: '#475569' }); }
                            const confirm = popup.querySelector('.swal2-confirm');
                            if (confirm) {
                                Object.assign(confirm.style, {
                                    background: '#EB5757',
                ...
                Livewire.on('swal:warning', ({ title, message }) => {
                    swalDialog.fire({
                        icon: 'warning',
                        title,
                        text: message,
