...
<div class="space-y-6">
    <!-- Filters -->
    <div
        class="p-6 rounded-3xl border border-gray-100 bg-surface flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
        <div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <select wire:model.live="filterSemesterId"
                class="rounded-xl border-gray-300 text-sm focus:ring-brand focus:border-brand font-medium text-navy shadow-sm">
            ...
            <select wire:model.live="filterClassId"
                class="rounded-xl border-gray-300 text-sm focus:ring-brand focus:border-brand font-medium text-navy shadow-sm max-w-[200px]">
            ...
            @if($filterClassId)
                <a href="{{ route('faculty.classes.export', ['classSection' => $filterClassId, 'format' => 'csv']) }}"
                    class="inline-flex items-center justify-center px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-1 transition-all">
                    <svg class="w-4 h-4 mr-2 text-success" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    CSV
                </a>
                <a href="{{ route('faculty.classes.export', ['classSection' => $filterClassId, 'format' => 'pdf']) }}"
                    target="_blank"
                    class="inline-flex items-center justify-center px-4 py-2 bg-brand text-white border border-transparent rounded-xl text-sm font-semibold shadow-sm hover:bg-brand-hover focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-1 transition-all">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <div
            class="bg-surface rounded-3xl p-6 border border-gray-100 shadow-sm flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold {{ $stats['rate'] < 80 ? 'text-error' : 'text-brand' }}">
                    {{ $stats['rate'] }}%</p>
            </div>
        </div>
        <div
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-surface rounded-3xl p-6 border border-gray-100 shadow-sm">
            ...
            <div class="relative h-72 w-full" x-data="{ 
                    chart: null,
                    initialLabels: @js($chartLabels),
                    ...
                    renderChart() {
                        ...
                        const labels = this.$wire?.chartLabels ?? this.initialLabels;
                        const data = this.$wire?.chartData ?? this.initialData;
        ...
        <div class="bg-surface rounded-3xl p-6 border border-gray-100 shadow-sm flex flex-col">
            <h3 class="text-lg font-bold text-navy mb-4 flex items-center gap-2">
                ...
                At-Risk Students (< 80%) </h3>
                    <div class="flex-1 overflow-y-auto pr-2 space-y-4 max-h-72">
                        @forelse($atRiskStudents as $atRisk)
                            <div
                                class="bg-error/5 rounded-2xl p-4 border border-error/10 flex items-center justify-between">
                                <div>
                                    <p class="font-bold text-navy">{{ $atRisk['student']->first_name }}
                                        {{ $atRisk['student']->last_name }}</p>
                                    <p class="text-xs text-gray-500 font-medium">
                                        {{ $atRisk['student']->identity_id ?? 'N/A' }}</p>
                                </div>
                                <div class="text-right">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-error/10 text-error">
                                        {{ $atRisk['rate'] }}%
                                    </span>
                        ...
                        @empty
                            <div class="text-center py-8">
                                <svg class="mx-auto h-12 w-12 text-success/50 mb-3" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
